...
@router.get("/")
async def read_all_products(created_by: str, page_size: int=20, page: int=0):
    session = database.get_db_session(engine)
    ...
    return Response(data, 200, "Products retrieved successfully.", error=False)

@router.get("/search/{pattern}")
async def read_product(pattern: str):
    session = database.get_db_session(engine)
    data = None
    error = False
    try:
        data = session.query(Product).filter(Product.name.like(f'%{pattern}%')).all()
    except Exception as ex:
        print("Error", ex)
        response_message = "Product Not found"
        error = True
